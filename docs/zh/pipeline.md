...
- **可选译者后记**：Review/Autofix 确定正式译文后，强模型先基于证据起草后记，再由第二次调用核查事实并做批判性修订。文章先介绍作者和用户提供的创作背景，再回到作品，对长处和局限提出具体判断。产物指纹覆盖正式译文、提示词、模型路由、术语库、分析和已核实补充材料；保存的后记只进入导出，不会成为章节或 Review 输入。
...
继续同一目录而不是新建。指纹不匹配时重新开始全书审校。命中审校块缓存或已完成初审记录时
跳过章节术语筛选；需要新 Reviewer 请求时，同章请求共享一次筛选得到的完整章节术语快照。
