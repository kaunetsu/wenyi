...
The shared transport treats `max_retries` as additional attempts. Local full-jitter exponential backoff has a 30-second cap; a valid server `Retry-After` remains an uncapped minimum floor, and a small positive-only jitter is added after that floor to avoid synchronized recovery. All waits continue through the invocation's cooperative cancellation and deadline mechanism rather than sleeping directly in a provider or stage.

Each started provider attempt emits `llm_transport_attempt_finished` after the provider request returns or raises. Its monotonic `attempt_elapsed_ms` excludes concurrency and quota queues and retry sleep. `attempt` remains global within one `call_id`; `route_index` starts at zero for the primary route, and `route_attempt` starts at one again for each explicit fallback. One additive `llm_call_finished` terminal event records the logical outcome and monotonic elapsed time across queueing, retries and fallbacks. Generic workload fields contain only sizes and request settings, never prompt or response text. A generic `RequestStopped` remains reported as `stopped` when its cause cannot be identified safely from the exception type.
