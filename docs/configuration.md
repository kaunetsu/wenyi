...
Provider SDK retries are disabled. Wenyi retries transient connections/timeouts, HTTP 408/409/429 and 5xx responses, and empty responses through one shared policy. `max_retries` counts additional attempts after the initial request. Without a valid server `Retry-After`, Wenyi uses full-jitter exponential backoff capped locally at 30 seconds. A valid `retry-after-ms` header, numeric `Retry-After`, or HTTP-date `Retry-After` provides a minimum wait floor and is never shortened by that local cap; Wenyi waits for the greater of the server delay and local backoff, then adds up to 10% positive-only jitter capped at 5 seconds to reduce synchronized retries. Retry backoff releases the connection permit and responds to cancellation. Ordinary 4xx errors are not retried. PDF's default MinerU import uses a separate `MINERU_API_KEY`; the optional BabelDOC HTTP bridge is independent of model routing.
...
`deadline_seconds` is an invocation-wide cooperative budget, not a hard deadline for one HTTP attempt or logical model request. Together with Ctrl+C it stops queued requests and backoff cooperatively. An in-flight SDK call can finish or reach its connection timeout; completed work is retained for resume. A stopped invocation gets a new budget on restart.
...
Events record the routing plan and request operation, model, provider, profile, connection, inference fingerprint, call ID and attempt. Transport-attempt events add monotonic request duration, global and per-route attempt identity, safe HTTP error metadata, and generic workload measurements (`input_bytes`, message count, JSON mode, and the resolved output cap) without storing prompts or responses. The terminal logical-call event records total monotonic elapsed time, including queue waits, retry backoff and explicit fallback, plus the final outcome and actual fallback count. `input_bytes` is the UTF-8 length of the generic messages JSON representation, not a tokenizer count or final HTTP body size. Full-book and Review usage updates are journaled in `usage-pending.json` before publication, so an interrupted local merge can recover without counting the increment twice. A process killed after remote acceptance but before local persistence can still leave unknown remote usage.
